...
assume that the user is in a resting pose, and do not incorporate that
information from that arm into that update iteration. This effect
means the model stops updating when the person's arms are at their
side.

\stnote{So are continuous gestures handled the same as continuous
  words?  How come we don't treat the null gesture as uniform over the
  four objects, but instead skip doing the update?  I think we should
  treat null gesture the same as null words.}
...
\stnote{Can you list model parameters and their exact values?  We
  should say what each model parameter was and where it came from. The
  covariance of the Gaussians, and the language model?  We don't have
  to list all the words in the language model, but we should say we
  trained a unigram model by writing descriptions of each object along
  with misidentifications from the speech recognition system.}
